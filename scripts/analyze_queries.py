"""WS-E: mine the SFT traces for query templates, the URL funnel, and outcomes.

Reads every trace under data/traces/ (contract 1.5 in notes/phase2_plan.md) and
prints a readable text report:
  ...
  uv run python scripts/analyze_queries.py                          # full report
  uv run python scripts/analyze_queries.py --model claude-sonnet-5  # one teacher
  uv run python scripts/analyze_queries.py --json out/agg.json      # + raw aggregates

Pure local and READ-ONLY on data/: it parses trace JSON already on disk -- no
network, no API keys, no writes except the optional --json path (which is
refused inside data/). A corpus build may be appending traces concurrently;
the file list is snapshotted at start and torn/vanished files are skipped
(build_corpus writes atomically, so this is rare) and counted in the header.
"""
...
def city_of(trace: dict) -> str:
    """The city half of the episode input, which build_corpus formats as
    "{name}, {city}"; falls back to the text after the last comma."""
...
def extract_calls(trace: dict) -> tuple[list[str], list[dict], int]:
    ...
    if not queries and not scrapes and not trace.get("messages"):
        ...
        # (contract 1.5); modes/failures are unknowable there.
...
def load_traces(traces_dir: Path, model: str | None) -> tuple[list[dict], int]:
    """Snapshot the *.json list once, then read whatever still parses -- a
    corpus build may be appending concurrently. Returns (traces, n_skipped)."""
    traces, skipped = [], 0
    for path in sorted(traces_dir.glob("*.json")):
        try:
            trace = json.loads(path.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError):
            skipped += 1  # vanished or torn mid-write; just note it
            continue
...
def parse_args():
    ...
    parser.add_argument("--traces-dir", type=Path,
                        ...
                        help="trace directory (default data/traces; read-only)")
    parser.add_argument("--model", default=None,
                        help="only analyze traces from this model id (e.g. claude-sonnet-5)")
...
def main():
    ...
    if not args.traces_dir.is_dir():
        sys.exit(f"no such traces dir: {args.traces_dir}")

    traces, skipped = load_traces(args.traces_dir, args.model)
    header = (f"===== WS-E query analysis: {len(traces)} traces from {args.traces_dir}"
              + (f" (model={args.model})" if args.model else "")
              + (f"; {skipped} unreadable/in-flight files skipped" if skipped else "")
              + " =====")
    print(header)
    if not traces:
    ...
    if args.json:
        ...
        agg["traces_dir"] = str(args.traces_dir)
