"""v2 SFT export: corpus.sqlite sft-split teacher traces -> a student-rendered dataset.

This is the v2 rebuild of scripts/build_sft.py. The ONLY thing that changed is the
INPUT SOURCE: traces now come from `corpus.sqlite` (via src/corpus.py) instead of
loose `data/traces/*.json` files, and rejection is a DB field (already filtered out
by `iter_traces(include_rejected=False)`) rather than a `reject_list.txt` parsed by
hand. The student re-render pipeline is UNCHANGED -- the DB stores the same Anthropic
content-block `messages` shape v1 wrote, so every transform below is verbatim from v1.

This is still the one seam where the teacher->student context-distillation swap
happens: each teacher trajectory (captured under the teacher system prompt) is
re-rendered under the **student** system prompt through Gemma 4's chat template and
consolidated into a single trainable `data/sft/<family>/train.jsonl` (one example per
line) that scripts/train_sft.py consumes.
...
writing a bad example.
  ...
  uv run python scripts/datasets/build_sft.py                # corpus sft split -> data/sft/gemma-4-e4b-it/train.jsonl
  uv run python scripts/datasets/build_sft.py --limit 10     # quick smoke over the first 10 traces

Output contract (one JSON object per line -- UNCHANGED from v1, train_sft.py reads
only `messages`):

  {
    ...
    "meta": {"model": "<teacher model>", "cache_version": <int|str>,
             "prompt_variant": "student", "trace_source": "teacher"|"student_dagger",
             "source_trace": "<trace_id>"}
  }

A sidecar `<out>.meta.json` records provenance (git sha, corpus md5, student prompt
variant, MAX_TOOL_CHARS, seed, example/found counts, source counts by trace_source).
...
def parse_args():
    ...
    p.add_argument("--family", default=DEFAULT_FAMILY,
                   help="student family key; sets the default --out path "
                        f"data/sft/<family>/train.jsonl (default {DEFAULT_FAMILY})")
    p.add_argument("--out", type=Path, default=None,
                   help="output jsonl (default data/sft/<family>/train.jsonl)")
    ...
    p.add_argument("--limit", type=int, default=None,
                   help="process only the first N sft-split traces (trace_id order) -- for quick tests")
    return p.parse_args()


def main():
    args = parse_args()
    out_path = args.out or (REPO_ROOT / "data" / "sft" / args.family / "train.jsonl")

    tokenizer = load_tokenizer()
    if tokenizer is None:
        sys.exit("could not load the Gemma tokenizer (gated -- needs an HF login/cache)")

    kept: list[dict] = []          # {row, found, trace_id, trace_source}
    ...
    # Rejection is a DB field now: iter_traces(include_rejected=False) already drops
    # rejected traces, and split='sft' enforces the split (no eval-leak by hand).
    with open_corpus(args.corpus, create=False) as cx:
        traces = cx.iter_traces(split="sft", include_rejected=False)
        if args.limit is not None:
            traces = itertools.islice(traces, args.limit)

        for trace in traces:
            n_scanned += 1
            rid = trace.get("restaurant_id")
            tid = trace.get("trace_id") or rid
            try:
            ...
            kept.append({"row": row, "found": found, "trace_id": tid,
                         "trace_source": trace.get("trace_source")})
            token_lengths.append(token_length(tokenizer, messages))

    # --- found=false ratio guard (seeded downsample) ---------------------
    false_traces = sorted(k["trace_id"] for k in kept if not k["found"])
    ...
    # --- write, preserving original (trace_id-sorted) order --------------
    out_path.parent.mkdir(parents=True, exist_ok=True)
    n_written = n_written_true = n_written_false = 0
    source_counts: dict[str, int] = {}
    with open(out_path, "w", encoding="utf-8") as fh:
        for k in kept:
            if k["trace_id"] in drop_set:
                continue
    ...
    meta = {
        ...
        "seed": args.seed,
        "max_found_false_frac": args.max_found_false_frac,
        "n_examples": n_written,
        "found_counts": {"true": n_written_true, "false": n_written_false},
    ...
    # --- summary ---------------------------------------------------------
    print("\n===== build_sft summary =====")
    print(f"traces scanned : {n_scanned}  (corpus sft split, not rejected)")
    ...
    print(f"written        : {n_written} -> {out_path}")
