...
it MUST NOT clip real menus. This script measures, over data/traces/*.json:
  ...
  3. CAP SIMULATION for C in {15k,20k,30k,40k,50k,75k}: (a) %found-episodes whose
     ...
     path scripts/build_sft.py uses (Gemma tokenizer), and estimate the per-episode
     token-length distribution + how many episodes fall under max_length=32768.
  4. a recommended MAX_TOOL_CHARS.

Nothing is mutated. Numeric findings are written to data/review/tool_chars_report.json.

  uv run python scripts/analyze_tool_chars.py
  uv run python scripts/analyze_tool_chars.py --limit 50   # quick smoke
  uv run python scripts/analyze_tool_chars.py --no-tokens  # skip the tokenizer pass
"""
...
# Driver
# ---------------------------------------------------------------------------
def load_traces(traces_dir: Path, limit: int | None):
...
def run_token_simulation(trace_objs, caps, max_length):
...
def main():
    ...
    ap.add_argument("--traces-dir", type=Path, default=REPO_ROOT / "data" / "traces")
    ap.add_argument("--out", type=Path,
                    default=REPO_ROOT / "data" / "review" / "tool_chars_report.json")
    ap.add_argument("--limit", type=int, default=None,
                    help="process only the first N traces (sorted) -- for smoke tests")
    ap.add_argument("--no-tokens", action="store_true",
                    help="skip the (slower) Gemma-tokenizer cap simulation")
    args = ap.parse_args()

    traces = list(load_traces(args.traces_dir, args.limit))
    if not traces:
        sys.exit(f"no traces in {args.traces_dir}")

    # --- parse everything once -------------------------------------------
    parsed = [(path, parse_trace(trace)) for path, trace in traces]
    ...
    if not args.no_tokens:
        ...
        per_cap, render_fail = run_token_simulation(
            [(path, trace) for (path, trace), _ in zip(traces, parsed)],
            CANDIDATE_CAPS, MAX_LENGTH_DEFAULT,
        )
